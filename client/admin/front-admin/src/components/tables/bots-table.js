...
`
    ...
    this.data.rows.forEach(register => {
      ...
      tableRegisterBodyList.appendChild(platformLi)
      platformLi.textContent = `Plataforma: ${register.platform}`
      ...
      tableRegisterBodyList.appendChild(descriptionLi)
      descriptionLi.textContent = `Descripción: ${register.description}`

      const tokenLi = document.createElement('li')
      tableRegisterBodyList.appendChild(tokenLi)
      tokenLi.textContent = `Token: ${register.token ? '*****' + register.token.slice(-4) : 'No definido'}`
